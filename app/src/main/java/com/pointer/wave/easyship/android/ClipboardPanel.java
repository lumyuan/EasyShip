...
public class ClipboardPanel {
    ...
    private void deleteLine(){
        ...
        for (int i = endRowNum; i >= startRowNum; i--) {
            Log.d("deleteRow", "deleteLine: " + i);;
            deleteRow(i);
        }
    }

    public void deleteRow(int s) {
        DocumentProvider documentProvider = _textField.createDocumentProvider();
        int start = documentProvider.getRowOffset(s);
        int end = documentProvider.getRowSize(s);
        int count = documentProvider.getRowCount();
        Log.d("deleteRow", "deleteRow: " + start + "/" + end + "/" + count);
        if (end <= 1 && s == count - 1) {
            return;
        }
        if (s == count - 1 && end > 1){
            _textField.selectText(false);
            _textField.moveCaret(start + end - 1);
            _textField.paste("\n");
        }
        documentProvider.deleteAt(start, end, 0);
        _textField.respan();
    }
